...
def createPlane(r=0.5, dr=0.1):
    ...
    return plane
def drawcubev2(lines, peak):
    points = [0] * len(peak)
    linesx = np.zeros((len(lines), 2))
    for i in range(len(peak)):
        points[i] = peak[i].p
    for j in range(len(lines)):
        linesx[j][0] = lines[j].m1
        linesx[j][1] = lines[j].m2
...
def updatecube_friction(line, peak, cT):
    fx = [0] * len(peak)
    fy = [0] * len(peak)
    fz = [0] * len(peak)
    groundflag = [0] * len(peak)
    for flagcount in range(len(peak)):
...
def main():
    csv_file = open('GA_Robot_Variablev3_2v2.csv', 'r')
    a = []
    b = []
    c = []
    d = []
    ...
    plane = createPlane()
    peak = massinitv3(1, [0.0, 0.0, 0.1],point_position)
    ...
    print(len(b))
    t_start = time.time()
    while (T<10000 and not ended):
        t = time.time()
        xtxxx = updatecube_friction(edges, peak, xtxxx)
        ...
        if t - t_prev > 1. / 120.:
            ...
            mass.points = o3d.utility.Vector3dVector(pts)
            vis.update_geometry(lsd)
            vis.update_geometry(mass)
            vis.poll_events()
            vis.update_renderer()
        T = T + 1
